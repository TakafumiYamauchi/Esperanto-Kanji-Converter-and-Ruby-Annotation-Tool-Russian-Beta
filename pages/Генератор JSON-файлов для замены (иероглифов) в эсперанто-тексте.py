# Генератор JSON-файлов для замены (иероглифов) в эсперанто-тексте.py (Streamlit特有のpagesフォルダに入れるコード)
#
# 【概要】
#  このページでは、「エスペラント文の漢字(ロシア語)置換」で使う
#  最終的な JSON ファイル(合并3個JSONファイル)を生成する機能を提供します。
#
# 【主な流れ】
#  1) CSVファイルを取り込み(エスペラント語根→漢字/ロシア語訳の対応)
...
#  - 最終的に「全域替换用のリスト」「局部文字替换用のリスト」「二文字词根替换用リスト」の3つを合并
...
#=====================================================================
# 1) ページ設定 & タイトル (ユーザー向け部分のみロシア語)
#=====================================================================
st.set_page_config(
    page_title="Инструмент создания JSON-файлов для замены (иероглифов) в эсперанто-тексте",
    layout="wide"
)
st.title("Создание JSON-файлов для замены (иероглифов) в тексте на эсперанто")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方)
#=====================================================================
with st.expander("Открыть описание использования", expanded=True):
    st.markdown("""
    #### Введение
    На этой странице вы можете создать JSON-файл для замены текста 
    (иероглифы / русский перевод и т.п.) в эсперанто, 
    а затем загрузить его (около 50MB, в зависимости от объёма данных).

    Основные шаги использования таковы:
    1. Загрузите **CSV-файл** (соответствие корней эсперанто и русского/иероглифов) 
       или используйте файл по умолчанию.
    2. При необходимости загрузите **JSON-файл** (правила разбора корней или 
       настройки для строк замены) или используйте файл по умолчанию.
    3. Нажмите «Создать JSON-файл для замены», чтобы скачать готовый
       **JSON-файл** для дальнейшего использования на главной странице.

    Ниже также доступны примеры файлов, которые можно загрузить и посмотреть,
    чтобы понять структуру и формат настроек.
    """)

#=====================================================================
# 3) サンプルファイル一覧 (折りたたみ) - (ユーザー向け文言のみ翻訳)
#=====================================================================
with st.expander("Список примеров файлов (для загрузки)"):
    st.write("#### Перечень примеров файлов")

    # Пример CSV1
    st.markdown("""
    **Пример CSV1 (корень эсперанто → русский перевод + руби)**  
    Каждый корень эсперанто в одной строке CSV сопоставлен с русским переводом (и руби).
    Создав CSV в таком же формате, вы сможете автоматически формировать 
    JSON-файл для замены.
    """)
    file_path0 = './Appの运行に使用する各类文件/Корни эсперанто – список с русским переводом и руби.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Скачать пример CSV1 (эсперанто → русский перевод + руби)",
            data=file,
            file_name="Пример_CSV1_эсперанто-русский_перевод_рубИ.csv",
            mime="text/csv"
        )

    # Пример CSV2
    st.markdown("""
    **Пример CSV2 (корень эсперанто → иероглифы, вариант Mingeo)**  
    Этот CSV-файл сопоставляет корни эсперанто и иероглифы (версия Mingeo).
    """)
    file_path0 = './Appの运行に使用する各类文件/Mingeo先生版 世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Скачать пример CSV2 (корень эсперанто → иероглифы, Mingeo)",
            data=file,
            file_name="Пример_CSV2_эсперанто-иероглифы(Mingeo).csv",
            mime="text/csv"
        )

    # Пример CSV3
    st.markdown("""
    **Пример CSV3 (корень эсперанто → иероглифы)**  
    Этот CSV содержит сопоставление корней эсперанто и иероглифов.
    """)
    file_path0 = './Appの运行に使用する各类文件/世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Скачать пример CSV3 (корень эсперанто → иероглифы)",
            data=file,
            file_name="Пример_CSV3_эсперанто-иероглифы.csv",
            mime="text/csv"
        )

    # Пример JSON1
    st.markdown("""
    **Пример JSON1 (Пользовательские настройки разбора корней эсперанто)**  
    **Назначение**: Определяет, как разбирать корни эсперанто, 
    когда подключать производные формы (глагольные окончания и т.п.), 
    с каким приоритетом их обрабатывать и т.д.
    """)
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Скачать пример JSON1 (Пользовательские настройки разбора корней)",
            data=file_json,
            file_name="Пример_JSON1_разбор_корней_эсперанто.json",
            mime="application/json"
        )

    # Пример JSON2
    st.markdown("""
    **Пример JSON2 (Пользовательские настройки итоговой строки замены)**  
    **Назначение**: Позволяет вручную привязать к определённому корню 
    конкретные иероглифы или особые обозначения. Как правило, хватает CSV и 
    настроек разбора корней, но при необходимости можно подробнее настроить замены.
    """)
    json_file_path2 = './Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json'
    with open(json_file_path2, "rb") as file_json:
        btn_json = st.download_button(
            label="Скачать пример JSON2 (Пользовательские настройки итоговой строки)",
            data=file_json,
            file_name="Пример_JSON2_строки_после_замены.json",
            mime="application/json"
        )

    # Пример Excel
    st.markdown("""
    **Пример Excel1 (корень эсперанто → японский перевод + руби + уровень освоения)**  
    **Назначение**: Полезно для настройки руби-перевода для конкретных корней.
    В таблице имеется также уровень освоения (по словарю «эсперанто-японский»).
    """)
    with open('./Appの运行に使用する各类文件/エスペラント語根-日本語訳ルビ対応リスト(習得レベル付き).xlsx', "rb") as file:
        st.download_button(
            label="Скачать пример Excel (эсперанто → японский перевод и руби)",
            data=file,
            file_name="Пример_Excel1_эсперанто-японский_рубИ_уровень.xlsx",
...
#=====================================================================
# ユーザーに見せる出力形式選択 (ユーザー向けラベルをロシア語化)
#=====================================================================
options = {
...
selected_display = st.selectbox('Выберите формат вывода:', display_options)
format_type = options[selected_display]

# Пример вывода (образец): main_text_list と ruby_content_list
...
st.write("---")
st.markdown("**Пример отформатированного текста ⇓**")
...
# ステップ１: CSVファイルを準備
#=====================================================================
st.header("Шаг 1: Подготовить CSV-файл")
st.markdown(
    """
### Пожалуйста, выберите **CSV-файл**, содержащий корни эсперанто и информацию о переводе (иероглифы)

---
    """
)

csv_choice = st.radio("Что делать с CSV-файлом?", ("Загрузить файл", "Использовать по умолчанию"))
csv_path_default = "./Appの运行に使用する各类文件/Корни эсперанто – список с русским переводом и руби.csv"

CSV_data_imported = None

if csv_choice == "Загрузить файл":
    st.write("Пожалуйста, загрузите нужный CSV-файл (рекомендуется UTF-8).")
    ...
    if uploaded_file is not None:
        ...
        st.warning("CSV-файл не загружен.")
        st.stop()

elif csv_choice == "Использовать по умолчанию":
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("Используется CSV-файл по умолчанию.")
    except FileNotFoundError:
        st.error("CSV-файл по умолчанию не найден. Операция остановлена.")
        st.stop()

st.write("Чтение CSV-файла завершено. Переходим к следующему шагу.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備
#=====================================================================
st.header("Шаг 2: Подготовить JSON-файл(правила разбора корней и т.п.)")
st.markdown("""
**JSON-файл**, описывающий методы разбора корней эсперанто или 
ваши собственные строки замены, можно загрузить или использовать по умолчанию.
Для примера можно посмотреть **JSON-пример**, отредактировать его
и загрузить обратно.
""")

# 1. エスペラント単語の語根分解法(カスタム)を指定する JSON
json_choice = st.radio("1. Что делать с JSON-файлом для разбора корней эсперанто?", ("Загрузить", "Использовать по умолчанию"))
json_path_default = "./Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json"
custom_stemming_setting_list = None

if json_choice == "Загрузить":
    uploaded_json = st.file_uploader("Пожалуйста, загрузите JSON-файл", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("JSON-файл успешно загружен.")
    else:
        st.warning("JSON-файл не загружен.")
        st.stop()
elif json_choice == "Использовать по умолчанию":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("Используется JSON-файл по умолчанию.")
    except FileNotFoundError:
        st.error("JSON-файл по умолчанию не найден.")
        st.stop()

# 2. 置換後文字列を追加指定する JSON
json_choice2 = st.radio("2. Что делать с JSON-файлом для добавления собственных строк замены?", ("Загрузить", "Использовать по умолчанию"))
json_path_default2 = "./Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json"
user_replacement_item_setting_list = None

if json_choice2 == "Загрузить":
    uploaded_json = st.file_uploader("Загрузите второй JSON-файл", type=['json'])
    if uploaded_json is not None:
...
# ステップ3: 高度な設定 (並列処理)
#=====================================================================
st.header("Шаг 3: Расширенные настройки (параллельная обработка)")
with st.expander("Настройки параллельной обработки"):
    st.write("""
    Здесь можно выбрать количество процессов, 
    используемых при создании JSON-файла для замены.
    При большом объёме текста и корней возможно ускорение 
    за счёт использования нескольких ядер CPU.
    """)

    use_parallel = st.checkbox("Использовать параллельную обработку", value=False)
    num_processes = st.number_input("Количество процессов одновременно", min_value=2, max_value=6, value=5, step=1)

st.write("### Создать финальный JSON-файл для замены (кнопка)")

#=====================================================================
# 置換用JSONファイルを作成するボタン
#=====================================================================
if st.button("Создать JSON-файл для замены"):
    with st.spinner("Формируется JSON-файл для замены... Пожалуйста, подождите."):
        ...
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"{current_count}/{total_items} строк обработано...")

            progress_bar.progress(100)
            progress_text.write("Наиболее длительный этап обработки завершён на 100% (пожалуйста, подождите ещё 3-4 секунды).")
        ...
        download_data = json.dumps(combined_data, ensure_ascii=False, indent=2)  # 実際には生成したデータを入れる
        st.success("Генерация списка для замены завершена!")

        st.download_button(
            label="Скачать итоговый список для замены (3 JSON в одном)",
            data=download_data,
            file_name="Итоговый_список_замены(слияние_3_JSON).json",
            mime='application/json'
        )